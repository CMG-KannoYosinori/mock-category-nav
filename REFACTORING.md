...
グローバルナビゲーション内のタブ要素に、タブ制御用のクラスと属性を追加しました。`.nav_slider` の代わりに `.p-global-nav__menu-list` でラップしています。

**変更箇所**: 約 348-350 行目

```html
<div class="p-global-nav__menu-list">
  <!-- タメせるタブ -->
...
タブの選択状態とコンテンツの表示/非表示を制御するためのスタイルを、`list_tamesu--swiper.css` に追加しています。

**変更箇所**: `s/app_published/list_tamesu--swiper.css`（`.tab-nav-item`, `.tab-content` など）
