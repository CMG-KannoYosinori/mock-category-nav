...
- **グローバルナビ**（`<nav id="header_nav_app">`、タメせる／メディタメのタブ）は、通常の Web だけでなく**アプリの WebView でも表示**されます。タブ切り替え時の Google Analytics 送信は、この要素が存在する（＝アプリ WebView）場合は行いません。
- 「アプリの場合は計測を除外する」ブロック（`#header_nav_app` が存在するときは `ga('send', 'event', ...)` を呼ばない処理）が不要と判断する場合は、削除して構いません。その場合、アプリ WebView でもタブ切り替えイベントが GA に送信されるようになります。削除前に計測・分析担当または仕様を決めた人に確認することを推奨します。
...
*カテゴリのメニュー（タブ）とコンテンツのスライド連動には、Swiper バージョン 8 の Controller による双方向同期を利用しています。*
